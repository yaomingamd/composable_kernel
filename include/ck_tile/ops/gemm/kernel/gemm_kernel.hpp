...
struct GemmKernel
{
                                                        ...
                                                        const index_t)
    {
        const auto& a_pad_view = [&]() {
            ...
            {
                return pad_tensor_view(a_tensor_view,
                                       make_tuple(M, TilePartitioner::KPerBlock),
                                       sequence<false, GemmPipeline::kPadK>{});
            }
            else
            {
                return pad_tensor_view(a_tensor_view,
                                       make_tuple(TilePartitioner::KPerBlock, M),
                                       sequence<false, GemmPipeline::kPadM>{});
            }
        }();
        ...
        const auto& b_pad_view = [&]() {
            ...
            {
                return pad_tensor_view(b_tensor_view,
                                       make_tuple(N, TilePartitioner::KPerBlock),
                                       sequence<false, GemmPipeline::kPadK>{});
            }
            else
            {
                return pad_tensor_view(b_tensor_view,
                                       make_tuple(TilePartitioner::KPerBlock, N),
                                       sequence<false, GemmPipeline::kPadN>{});
            }
        }();
        ...
        const auto& c_pad_view = [&]() {
            ...
            if constexpr(std::is_same_v<CLayout, tensor_layout::gemm::RowMajor>)
            {
                return pad_tensor_view(
                    c_tensor_view,
                    make_tuple(TilePartitioner::MPerBlock, TilePartitioner::NPerBlock),
                    sequence<false, GemmPipeline::kPadN>{});
            }
            else
            {
        ...
        const PadView& views, const index_t i_n, const index_t i_k, [[maybe_unused]] const index_t M, [[maybe_unused]] const index_t N, [[maybe_unused]] const index_t K)
    {
        ...
        const auto& a_block_window = [&]() {
            if constexpr(std::is_same_v<ALayout, tensor_layout::gemm::RowMajor>)
            {
                return make_tile_window(
                    a_pad_view, make_tuple(TilePartitioner::MPerBlock, TilePartitioner::KPerBlock), {0, i_k});
            }
            else
            {
                return make_tile_window(
                    a_pad_view, make_tuple(TilePartitioner::KPerBlock, TilePartitioner::MPerBlock), {i_k, 0});
            }
        }();
        ...
        const auto& b_block_window = [&]() {
            if constexpr(std::is_same_v<BLayout, tensor_layout::gemm::ColumnMajor>)
            {
                return make_tile_window(
                    b_pad_view,
                    make_tuple(TilePartitioner::NPerBlock, TilePartitioner::KPerBlock),
                    {i_n, i_k});
            }
            else
            {
                return make_tile_window(
                    b_pad_view,
                    make_tuple(TilePartitioner::KPerBlock, TilePartitioner::NPerBlock),
                    {i_k, i_n});
            }
        }();

        auto c_block_window =
            make_tile_window(c_pad_view,
                             make_tuple(TilePartitioner::MPerBlock, TilePartitioner::NPerBlock),
                             {0, i_n});
                                            ...
                                            const index_t block_idx_k)
    {
        ...
        const auto& gemm_pad_views =
            MakePingPongGemmPadViews(gemm_tensor_views_tuple, kargs.M, kargs.N, kargs.K);
        ...
        GemmPipeline{}.template operator()(
            a_block_window, b_block_window, c_block_window, num_loop, smem_ptr_0, EpilogueFunc);
    }
