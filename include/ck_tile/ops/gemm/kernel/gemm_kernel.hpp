...
///      NumDTensor describes the number of A tensors. The minimum number of tensors is 1
///      (required). NumDTensor describes the number of B tensors. The minimum number of tensors is
///      1 (required). NumDTensor describes the number of D tensors. The minimum number of tensors
///      is 0 (not required).
...
struct GemmKernel
{
    ...
    using KernelArgs = GemmKernelArgs<AsLayout::size(), BsLayout::size(), DsLayout::size()>;

    [[nodiscard]] CK_TILE_HOST static const std::string GetName()
    {
        using AiDataType = remove_cvref_t<std::tuple_element_t<number<0>{}, AsDataType>>;
        using BiDataType = remove_cvref_t<std::tuple_element_t<number<0>{}, BsDataType>>;
        // clang-format off
        return concat('_', "gemm", gemm_prec_str<AiDataType, BiDataType>, GemmPipeline::GetName());
        // clang-format on
    }
    ...
    MakeKernelArgs(const GemmHostArgs<NumATensor, NumBTensor, NumDTensor>& hostArgs)
    {
