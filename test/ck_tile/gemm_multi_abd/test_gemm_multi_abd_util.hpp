...
struct AddScale
{
    ...
    operator()(E& e, const A& a0, const A& a1) const
    {
        const float x0_f = ck_tile::type_convert<float>(scale * (a0 + a1));
...
struct AlphaBetaAdd
{
    ...
    operator()(E& e, const A& a0, const A& a1) const
    {
        const float x0_f = ck_tile::type_convert<float>(alpha_ * a0 + beta_ * a1);
        e = ck_tile::type_convert<E>(x0_f);
    };
...
class TestCkTileGemmMultiABD : public ::testing::Test
{
    ...
    void invoke_gemm_multi_abd(const ck_tile::GemmHostArgs<AsDataType::size(), BsDataType::size(), DsDataType::size()>& args,
             ...
             int StrideE   = 0)
    {
        ...
        StrideA0 = f_get_default_stride(M, N, StrideA0, A0Layout{});
        StrideA1 = f_get_default_stride(M, N, StrideA1, A1Layout{});
        ...
        ck_tile::GemmHostArgs<AsDataType::size(), 
                              BsDataType::size(), 
                              DsDataType::size()> args({as_ptr_buf,
                                                        bs_ptr_buf,
                                                        ds_ptr_buf,
        ...
        std::cout << "Run kernel with M =" << M << " N =" << N << " K =" << K
                  << " StrideA =" << StrideA0 << " StrideB =" << StrideB0 << " StrideE =" << StrideE
        ...
        ck_tile::HostTensor<A0DataType> a_m_k_host_ref_element_result(f_host_tensor_descriptor(M, K, StrideA0, A0Layout{}));
        ck_tile::HostTensor<B0DataType> b_k_n_host_ref_element_result(f_host_tensor_descriptor(K, N, StrideB0, B0Layout{}));
        ck_tile::HostTensor<EDataType> e_m_n_host_ref(f_host_tensor_descriptor(M, N, StrideE, ELayout{}));
        ...
        ck_tile::reference_gemm_multiple_abd<AsDataType,
                                           ...
                                           BElementWiseFn,
                                           CDElementWiseFn>(
            {a0_m_k_tesnor, a1_m_k_tesnor}, {b0_k_n_tensors, b1_k_n_tensors}, {d0_m_n_tensors, d1_m_n_tensors}, a_m_k_host_ref_element_result, b_k_n_host_ref_element_result, e_m_n_host_ref);
